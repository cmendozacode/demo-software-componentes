...
import { currentUserPermissions, AccesoModel } from '../../../core/auth';
...
export class BaseComponent implements OnInit, OnDestroy {
	...
	private currentUserPermissions$: Observable<AccesoModel[]>;
	...
	loadRolesWithPermissions() {
		...
		//Agregar Permiso Generico
		this.permissionsService.addPermission('accessToGenericDummy');
		...
		const subscr = this.currentUserPermissions$.subscribe(res => {
			...
			//Agregar Permiso Generico
		    this.permissionsService.addPermission('accessToGenericDummy');
			//populated all permissions from userPermision
			res.forEach((acc: AccesoModel) => this.permissionsService.addPermission(acc.perfilAccesoTag));
		});
		this.unsubscribe.push(subscr);
	}
}
