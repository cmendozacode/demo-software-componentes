...
from BusinessObject.single_sign_on import SingleSignOnBO
from BusinessObject.remote_query import RemoteQueryBO
...
def lambda_handler(event, context):
    ...
    sqlconnection         = os.environ["DATABASE_CONNECTION"]
    ...
    #Ejecutar Metodo Get - Single SignOn
    if http_method_name == 'singlesignon' and httpBodyJson is not None and valueTokenValidFlag and object_name is not None:
        print("singlesignon")
        object_data_connector = SingleSignOnBO()
 
        reponseObject = object_data_connector.fn_get_software_auth_sql(httpBodyJson,object_name)
        
    
    #Ejecutar Metodo Get - Obtener Querys Remotos
    if http_method_name == 'remotequery' and httpBodyJson is not None and valueTokenValidFlag and object_name is not None:
        print("remotequery")
        object_data_connector = RemoteQueryBO()
 
        reponseObject = object_data_connector.fn_get_query_sql(httpBodyJson,object_name)
