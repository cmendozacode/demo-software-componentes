...
#Importar Solo librerias Utilizadas
microsoft_sql_connector = None
dynamodb_connector = None
aws_athenas_sql_connector = None

for curConnector in objDataSysStructure.current_data_connectors:
    
    if curConnector == "microsoft_sql":
        from data_connector.microsoft_sql import microsoft_sql_connector

    if curConnector == "dynamodb":
        from data_connector.dynamodb import dynamodb_connector

    if curConnector == "aws_athenas":
...
class data_connector():
    ...
    def __init__(self, obj_name, str_connection: Optional[str] = None, str_table_index: Optional[str] = None, str_region: Optional[str] = None, str_output_sql_athenas: Optional[str] = None):
        ...
        if self.data_structure['connector'] == "aws_athenas":
            self.obj_cur_data_connection = aws_athenas_sql_connector(self.data_structure,str_region,str_output_sql_athenas)
